    ...
    <div class ="article-item-inner">
        <div class = "article-body">
            <div class="user-info">
                ...
                <el-divider direction="vertical"></el-divider>
                <router-link :to="`/tips/${data.pBoardId}`" class="link-info">{{ data.pBoardName }}</router-link>
                <template v-if="data.boardName">
                    <span>&nbsp;/&nbsp;</span> 
                    <router-link :to="`/tips/${data.boardId}`" class="link-info">{{ data.boardName }}</router-link>
                </template>
            </div>
            <router-link :to="`/article/${data.articleId}`" class="title">
        ...
        <router-link :to="`/article/${data.articleId}`" class="title"><Cover :cover="data.cover" :width="100" v-if="data.cover"></Cover></router-link>
    </div>
  </div>
</template>
