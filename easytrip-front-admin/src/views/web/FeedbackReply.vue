      ...
      <div class="feedback-list">
        <div class="feed-item" v-for="data in dataList">
          ...
          <div class="feed-content-other" v-else>
            <div class="icon">{{ data.sendNickName.substr(0, 1) }}</div>
            <div class="content-panel">
              <div class="nick-name">{{ data.sendNickName }}</div>
              <div class="time">{{ data.createTime }}</div>
              <div class="content">
                {{ data.messageContent }}
              </div>
...
const api = {
  loadFeedbackReply: "/webFeedback/loadFeedbackReply",
...
const loadDataList = async () => {
  ...
  if (goBottom.value) {
    nextTick(() => {
      ...
      console.log(document.querySelector(".feedback-list").scrollHeight);
    });
  }
};
